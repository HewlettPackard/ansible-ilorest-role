...
DOCUMENTATION = '''
---
module: ex20_get_ilo_nic
short_description: This module gets the iLO NIC configuration
'''
...
#Main function
def get_ilo_nic(redfishobj, get_active):
    ...
    for instance in instances:
        ...
        for entry in response.dict["Members"]:
            ...
            if get_active and nic.dict["Status"]["State"] == "Enabled":
                tmp_dict["Status"] = ("Active" + entry["@odata.id"] + ": " + \
                                                    json.dumps(nic.dict))
            elif get_active == False and nic.dict["Status"]["State"] == \
                                                                    "Disabled":
                tmp_dict["Status"] = ("InActive" + entry["@odata.id"] + ": " + \
                                                    json.dumps(nic.dict))
    return tmp_dict
...
def main():
    ...
    get_active = module.params['get_active']
    ...
    try:
        ...
        raise excp
    display_dict = get_ilo_nic(REDFISH_OBJ, get_active)

...
#Main function
def computer_details(redfishobj):
    ...
    for instance in instances:
        ...
        tmp_dict["Manufacturer"] = str(response.dict["Manufacturer"])
        tmp_dict["Model"] = str(response.dict["Model"])
        tmp_dict["SerialNumber"] = str(response.dict["SerialNumber"])
        if "VirtualSerialNumber" in response.dict:
            tmp_dict["VirtualSerialNumber"] = str(response.dict\
                                        ["VirtualSerialNumber"])
        else:
            tmp_dict["VirtualSerialNumber"] = ("Virtual Serial Number information not " \
                                        "available on system resource")
        tmp_dict["UUID"] = str(response.dict["UUID"])

        if "VirtualUUID" in response.dict["Oem"]["Hp"]:
            tmp_dict["VirtualUUID"] = str(response.dict["Oem"]\
                                          ["Hp"]["VirtualUUID"])
        else:
            tmp_dict["VirtualUUID"] = "VirtualUUID not available system " \
                                                            "resource"
        if "AssetTag" in response.dict:
            tmp_dict["AssetTag"] = response.dict["AssetTag"]
        else:
            tmp_dict["AssetTag"] = "No Asset Tag information on system " \
                                                            "resource"
        tmp_dict["BIOS Version"] = response.dict["BiosVersion"]

        tmp_dict["Memory"] = (str(response.dict["MemorySummary"]\
                                  ["TotalSystemMemoryGiB"]) + " GB\n")

        tmp_dict["Processors"] = (str(response.dict\
                            ["ProcessorSummary"]["Count"]) + " x " + \
                     str(response.dict["ProcessorSummary"]["Model"]))

        if "Status" not in response.dict or "Health" not in \
                                                    response.dict["Status"]:
            tmp_dict["Status"] = ("Status/Health information not available in "
                                                        "system resource")
        else:
            tmp_dict["Status"] = str(response.dict["Status"]["Health"])

        if "HostCorrelation" in response.dict:
            if "HostFQDN" in response.dict["HostCorrelation"]:
                tmp_dict["HostFQDN"] = response.dict["HostCorrelation"]\
                                       ["HostFQDN"]
                
            if "HostMACAddress" in response.dict["HostCorrelation"]:
                for mac in response.dict["HostCorrelation"]["HostMACAddress"]:
                    tmp_dict["HostMACAddress"] = str(mac)

            if "HostName" in response.dict["HostCorrelation"]:
                tmp_dict["HostName"] = response.dict["HostCorrelation"]\
                                       ["HostName"]
            ...
            if "IPAddress" in response.dict["HostCorrelation"]:
                for ip_address in response.dict["HostCorrelation"]\
                                                            ["IPAddress"]:
                    if ip_address:
                        tmp_dict["HostIPAddress"] = str(ip_address)
...
def main():
    ...
    exit_msg = computer_details(REDFISH_OBJ)
    module.exit_json(changed=True, msg=exit_msg)

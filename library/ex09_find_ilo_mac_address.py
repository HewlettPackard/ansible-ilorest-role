...
#Main function
def find_ilo_mac_address(redfishobj):
    ...
    for instance in instances:
        ...
        for entry in response.dict["Members"]:
            ...
            if "MACAddress" not in ethernet.dict:
                return("NIC resource does not contain " \
                                                    "'MACAddress' property")
            else:
                return(ethernet.dict["Name"] + " = " + \
...
def main():
    ...
    exit_msg = find_ilo_mac_address(REDFISH_OBJ)

...
#Main function
def get_schema(redfishobj, schema_prefix):
    tmp_dict = {}
    ...
    for entry in response.dict["Members"]:
        ...
        if schema.dict["Schema"].startswith(schema_prefix):
            for location in schema.dict["Location"]:
                ...
                if response.status == 200:
                    tmp_dict["Status"] = ("Found " + schema_prefix + " at "\
                                                        + extref_uri)
                    
                else:
                    tmp_dict["Status"] = (schema_prefix + " not found at " \
                                                        + extref_uri)
...
def main():
    ...
    try:
        ...
        raise excp
    display_dict = get_schema(REDFISH_OBJ)

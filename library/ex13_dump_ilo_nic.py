...
DOCUMENTATION = '''
---
module: ex13_dump_ilo_nic
short_description: This module dumps nic information from iLO
'''
        ...
        for entry in response.dict["Members"]:
            ...
            if nic.dict["Status"]["State"] == "Enabled":
                ...
                if "MACAddress" not in nic.dict:
                    ...
                    tmp_dict["MACAddress"] = (str(nic.dict["MACAddress"]))
                ...
                if not "Autosence" in nic.dict:
                    ...
                    tmp_dict["Autosense"] = (str(nic.dict["Autosense"]))
                ...
                if "FQDN" not in nic.dict:
                    ...
                    tmp_dict["FQDN"] = str(nic.dict["FQDN"])

                for addr in nic.dict["IPv4Addresses"]:
                    tmp_dict["IPv4Addresses"] = (addr["Address"] 
                ...
                if "IPv6Addresses" not in nic.dict:
                    ...
                    for addr in nic.dict["IPv6Addresses"]:
                        tmp_dict["IPv6Addresses"] = (addr["Address"] 
                               + " from " + addr["AddressOrigin"])
    return tmp_dict
...
def main():
    ...
    try:
        ...
        raise excp
    display_dict = dump_ilo_nic(REDFISH_OBJ)
    module.exit_json(changed=True, msg=display_dict)
